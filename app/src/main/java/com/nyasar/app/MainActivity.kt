...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            // actually-applied locale, triggering an Activity recreate that
            // resets settings to null again, causing an infinite loop.
            LaunchedEffect(settings?.languageMode) {
                val s = settings ?: return@LaunchedEffect
                val localeList = when (s.languageMode) {
                    "id" -> androidx.core.os.LocaleListCompat.forLanguageTags("id")
                    "en" -> androidx.core.os.LocaleListCompat.forLanguageTags("en")
                    else -> androidx.core.os.LocaleListCompat.getEmptyLocaleList()
                }
                if (androidx.appcompat.app.AppCompatDelegate.getApplicationLocales() != localeList) {
