...
object RasterStyleJson {
     ...
     *
     * Nyasar builds this inline so the basemap does not depend on
     * styles.gpx.studio and does not silently fall back to Esri Satellite.
     *
     * Imagery source URL is intentionally blank by default: Nyasar does not
     * ship a confirmed free imagery source in this pass, so this style
     * currently renders the vector overlay only. When a real imagery source
     * is configured, pass its tile URL here — that is the single place to
     * change.
     */
